...
If you are currently using L<Starch::Store::Amazon::DynamoDB> you can use
a layered store to migrate from the old table to a new table, as in:

    store => {
        class => '::Layered',
        outer => {
            class => '::Paws::DynamoDB',
            table => 'new_sessions_table',
            ...
        },
        inner => {
            class => '::Amazon::DynamoDB',
            table => 'old_sessions_table',
            ...
        },
    },

Then once enough time has passed you can get rid of the inner store, and
it's table.
